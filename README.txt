...
The built image is then used to create a container which will execute the code with prebuilt dependencies.
...
- We send off both a HTML and plaintext version of the email to allow it to be compatible with different browsers.
...
- The container starts off as a root user but finishes as a nobody user so there are no permissions left in the container, in case someone
is able to get in.
