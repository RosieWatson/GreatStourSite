...
Constructs dependancies, installs everything, prepares the distribution friendly version of the code and exposes ports ready to listen
`docker run -p 3000:3000 iot-g7:latest`
The built image is then used to create a container will execute the code with prebuilt dependencies.
...
- antd: This is a front-end component library. We used chose to use this as it provided us with a large choice of ready built, tried and tested
    components which could help us create an attractive and accessible UI easily. It is designed to be used with React and gave us a vast functionality
...
- Skip to the main content and skip over map content links to aid keyboard/screenreader users to navigate
- Content in the interactive map is available in an alternative format (in text via the sidebar)
- The content displayed in the charts are also available in text format
...
- The content contained in a single dashboard which is easy to way-find
...
- We send off both an HTML and plain text version of the email to allow it to be compatible with different browsers.
- We use Google's Invisible Recaptcha to prevent spam or bot attacks for subscribing and unsubscribe points. This Recaptcha will only display an input Recaptcha (requiring user action), if it cannot determine that the user is human.

Security Considerations:
- We have built a database library that is safe from SQL injection.
- We have stored any private keys as environment variables and not committed them to the repo at any point.
...
- The container starts off as a root user but finishes as a nobody user so there are no permissions left in the container, in case someone is able to get in.
